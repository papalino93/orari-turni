...
export function CopyWeekButton({
  ...
  /** Dipendente selezionato in "Impostazioni vista", se c'e: fa da scelta iniziale. */
  ...
  const targetName = employees.find((e) => e.id === target)?.name ?? null;

  function confirmCopy() {
    startTransition(async () => {
      const result = await runWithToast(toast, () => copyPreviousWeek(weekStartKey, target || undefined), undefined);
      if (!result) return;
      ...
      const who = targetName ? ` per ${targetName}` : "";
      if (result.sourceEmpty) {
        toast.showSuccess(`La settimana precedente è vuota${who}: non c'è nulla da ripetere.`);
          ...
          <div className="flex min-h-full items-end justify-center sm:items-center sm:p-4">
            ...
            >
              ...
              <label className="mt-4 block">
                <span className="mb-1.5 block text-xs font-medium text-foreground-muted">Per chi</span>
                <select
                  value={target}
